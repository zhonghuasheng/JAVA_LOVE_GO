### Summary
Collections是一个提供多种静态方法的集合类，方便了对集合的操作。
* Collections.sort(integerList);
* Collections.addAll(stringList, "c", "d");
* Collections.copy(dest, stringList);
* Collections.max(integerList) // 最大值
* Collections.min(integerList) // 最小值
* Collections.reverse(integerList); // 反转
* Collections.emptyList(); // 返回空值
* Collections.synchronizedList(dest); // 返回一个synchronized的集合
